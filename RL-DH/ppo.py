...
def ppo(env_fn, actor_critic=core.MLPActorCritic, ac_kwargs=dict(), seed=0, 
        ...
        target_kl=0.01, logger_kwargs=dict(), save_freq=1, filter_type=0, lagrangian=False, pret_dir=''):
    """
    ...
    if filter_type == 1:
        ...
        filter = nn_filter.get_hyp
    elif filter_type == 2: # pretrained hyperplane with frozen gradients
        old_ac = torch.load(pret_dir)
        mu_net = old_ac.pi.hyp
        a_dim = act_dim[0]
        for param in mu_net.parameters():
            param.requires_grad = False
        filter = lambda obs : (mu_net(obs)[:, :-1], mu_net(obs)[:, -1:]) if len(obs.shape) >= 2 else (mu_net(obs)[:-1], mu_net(obs)[-1:])
    elif filter_type == 3:
        old_ac = torch.load(pret_dir)
        hyp = old_ac.pi.hyp
        a_dim = act_dim[0]
        for param in hyp.parameters():
            param.requires_grad = False
        ac.pi.hyp = hyp
    ...
    def compute_loss_pi(data):
        ...
        if filter_type == 1 or filter_type==2:
            a_hyp, b_hyp = filter(obs) # get pretrained hyperplane from filter
            pi, logp = ac.pi(obs, act, a_hyp, b_hyp.view(-1, 1)) # filter mean pre sample
        else:
    ...
    for epoch in range(epochs):
        for t in range(local_steps_per_epoch):
            ...
            if filter_type==1 or filter_type==2:
                a_hyp,b_hyp = filter(torch.as_tensor(o, dtype=torch.float32))
                a, v, vc, logp = ac.step(torch.as_tensor(o, dtype=torch.float32), a_hyp, b_hyp)
            elif filter_type == 3:
                a, v, vc, logp = ac.step(torch.as_tensor(o, dtype=torch.float32))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--pret_dir', type=str, default='')
    ...
    if args.exp_name == 'ppo':
        filter = 0
        lagrangian = False
    elif args.exp_name == 'ppo_nn_qp' and args.env == 'CartPole':
        actor_critic = core.TruncMLPActorCritic
        filter = 1
        lagrangian = False
    elif args.exp_name == 'ppo_lag':
        filter = 0
        lagrangian = True
    elif args.exp_name == 'pret_ppo':
        if args.env == 'HalfCheetah':
            actor_critic = core.DiscMLPActorCritic
            filter = 3
        else:
            filter = 2
            actor_critic = core.TruncMLPActorCritic
        ...
        raise NotImplementedError('Exp name not implemented')
    ...
    ppo(env_fn=env_fn, actor_critic=actor_critic ,
        ...
        logger_kwargs=logger_kwargs, filter_type=filter, lagrangian=lagrangian, pret_dir=args.pret_dir)
